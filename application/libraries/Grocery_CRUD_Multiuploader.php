...
/**
 * PHP Grocery CRUD Multiuploader library
 */
class Grocery_CRUD_Multiuploader extends grocery_CRUD
{
	...
	protected $multiupload_css_path        = 'assets/multiuploader/styles/';

	/* multiupload js directory */
	protected $multiupload_javascript_path = 'assets/multiuploader/scripts/';

	/* Default upload directory */
	protected $path_to_directory           = 'assets/multiuploader/uploads';
		...
		if (!empty($files)) {
			...
			foreach ($files as $items) {
				...
				if ($this->remote_file_exists($thisfile)) {
					...
					if (($this->enable_download_button &&
							is_null($this->download_allowed)) ||
						($this->enable_download_button &&
							in_array(
								pathinfo(
